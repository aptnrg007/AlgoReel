// Mirrors renderTimeSeries.ts exactly (PLAN.md §27's recipe, item 8's "a
// demo spec + a real render"). Takes a bare BarRaceSpec (JSON) or a plain
// CSV, runs it through validate -> check -> render, and writes a real mp4
// next to the input file.
import { existsSync, mkdirSync, readFileSync } from "node:fs";
import { basename, dirname, extname, join, resolve } from "node:path";

import { renderVideo } from "../mcp/renderVideo";
import { toBarRaceVideoPlan } from "../plan/fromBarRaceSpec";
import { checkBarRaceRender } from "../spec/barRace/checkRender";
...
const USAGE =
  'usage: renderBarRace.ts <path-to-spec.json | path-to-data.csv> [--duration=20] [--out=path.mp4]\n' +
  "  CSV input additionally requires: --title=... --x-label=... --value-label=...";
...
async function main(): Promise<void> {
  ...
  if (extname(resolvedInput).toLowerCase() === ".csv") {
    const missing = ["title", "x-label", "value-label"].filter((f) => !flags[f]);
    if (missing.length > 0) {
    ...
    try {
      candidate = parseCsvToBarRaceSpec(text, {
        title: flags.title!,
        xAxisLabel: flags["x-label"]!,
        valueLabel: flags["value-label"]!,
      });
    } catch (err) {
  ...
  const renderCheck = checkBarRaceRender(spec, targetDurationSec);
  ...
  const plan = toBarRaceVideoPlan(spec, { targetDurationSec, description: flags.description });
