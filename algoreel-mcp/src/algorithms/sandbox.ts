...
// Same isolation reasoning as GENERATED_GRAPH_DIR's own comment — kept in
// its own directory so rebuildTreeManifest()'s scan never mixes in an
...
// why this file's graph section above still uses a reference order).
...
// sandbox.ts's tree codegen section) before being cached here. From this
// point on it's a real, permanent algorithm file, run in-process like
// any hand-written one — no further sandboxing on load.
...
// Tree-side twin of rebuildManifest()/rebuildGraphManifest() — same
// static-import constraint, kept in its own directory/manifest so this
// scan never mixes in an array- or graph-shaped file.
